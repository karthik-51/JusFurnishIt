...
const ContactRequestPage = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "phoneNumber") {
      ...
      setPhoneError(phoneRegex.test(value) ? "" : "Phone number must be exactly 10 digits.");
    }
  
    // Scheduled Date validation
    if (name === "scheduledDate" || name === "bookingDate") {
      const newBookingDate = name === "bookingDate" ? value : formData.bookingDate;
      const newScheduledDate = name === "scheduledDate" ? value : formData.scheduledDate;
  
      if (newBookingDate && newScheduledDate && newScheduledDate < newBookingDate) {
        setFeedback("Scheduled date cannot be earlier than booking date.");
        return;
      } else {
        setFeedback(""); // Clear if valid
      }
    }
